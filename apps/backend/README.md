...
**Полный GPU-сервис:** корневой `Dockerfile` и `docker compose up --build`
из корня репозитория. Точка входа — `app.ensemble:app`.
Три модели и train-словарь подключены; см. [serving](../../docs/SERVING.md).
Ниже описан отдельно тестируемый HTTP-каркас `app.main:app` без весов.

## Локальный запуск

Команды выполняются из `apps/backend/`: сначала `cd apps/backend` из корня проекта.
Нужен Python 3.10+.
...
## Docker только для HTTP-каркаса, без модели
