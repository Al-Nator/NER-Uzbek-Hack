...
```text
frontend/                      React/TypeScript-интерфейс анализа текста
backend/                       FastAPI, HTTP-контракт и проверки
compose.yaml                   совместный запуск интерфейса и API
...
Интерфейс: <http://localhost:3000>. Swagger: <http://localhost:8000/docs>.
Для остановки используйте `docker compose down`.

Frontend поддерживает свой текст, случайные записи train/dev и файлы TXT/JSON/JSONL,
подсветку `NAME`, `ORG`, `GEO`, фильтры и экспорт JSON. Шрифты, оформление и снимок
текстов датасетов поставляются локально. API доступен через тот же origin.

Локальная разработка и проверки: [frontend/README.md](frontend/README.md) и
[backend/README.md](backend/README.md). Backend использует отдельное окружение;
контейнеры собираются из каталогов соответствующих сервисов.
